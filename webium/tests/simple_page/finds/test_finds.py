from tests import get_url
from tests.simple_page import SimplePageTest
from webium.common.by import By
from webium.find import Finds


class TestClick(SimplePageTest):
    def test_basic_finds(self):
        assert self.page.paragraphs[4].get_attribute('id') == 'nbspandspaces'

    def test_dynamic_find(self):
        paragraphs = Finds(by=By.TAG_NAME, value='p', context=self.page.physical_container)
        assert paragraphs[1].text == 'after pre'

    def test_custom_elements(self):
        anchors = self.page.anchor_list
        assert list(filter(lambda x: x.get_href(), anchors))[0].get_href() == get_url('resultPage.html')

    def test_is_element_present(self):
        divs = self.page.div_list
        assert divs[11].is_element_present('first_link')
        assert not divs[3].is_element_present('first_link')

    def test_inner_search(self):
        assert self.page.div_list[11].first_link.get_href(), get_url('resultPage.html')
